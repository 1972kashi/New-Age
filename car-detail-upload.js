(function(){
	...
	async function extractExistingCars() {
		try {
			...
			existingCars = Array.isArray(data.items) ? data.items : Array.isArray(data) ? data : [];
			updateCarNameList();
		} catch (err) {
	...
	async function extractExistingCarDetails() {
		try {
			...
			if (!res.ok) return;
			const data = await res.json();
			existingCarDetails = Array.isArray(data.items) ? data.items : Array.isArray(data) ? data : [];
			updateCarNameList();
		} catch (err) {
	...
	window.uploadListing = async function() {
		...
		const explicitImagePath = document.getElementById('imgPath').value.trim();
		const uploadedImagePaths = [];
		for (const file of uploadedFiles) {
			const uploadedPath = await uploadImageToServer(file);
			if (uploadedPath) uploadedImagePaths.push(uploadedPath);
		}
		const primaryImagePath = explicitImagePath || uploadedImagePaths[0] || '';
		const imagePaths = [];
		if (explicitImagePath) imagePaths.push(explicitImagePath);
		imagePaths.push(...uploadedImagePaths);
		const detailItem = {
			name: carName,
			...
			images: Array.from(new Set(imagePaths.filter(Boolean))),
			badge: true
		};
